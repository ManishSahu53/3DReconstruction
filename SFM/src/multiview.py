...
class PlaneKernel:
    ...
    def sampling(self):
        ...
        if len(self.vectors) > 0:
            samples['points'] = self.points[random.sample(
                range(len(self.points)), 2), :]
            samples['vectors'] = [self.vectors[i]
                                  for i in random.sample(range(len(self.vectors)), 1)]
        else:
    ...
    def fit(self, samples):
        model = fit_plane(samples['points'],
                          samples['vectors'], self.verticals)
        return [model]
    ...
    def evaluate(self, model):
        ...
        point_error = np.abs(model.T.dot(
            homogeneous(self.points).T))/normal_norm
        vectors = np.array(self.vectors)
        vector_norm = np.sum(vectors*vectors, axis=1)
        vectors = (vectors.T / vector_norm).T
        vector_error = abs(np.rad2deg(
            abs(np.arccos(vectors.dot(normal)/normal_norm)))-90)
        vector_error[vector_error < self.vector_threshold] = 0.0
        vector_error[vector_error >=
                     self.vector_threshold] = self.point_threshold+0.1
        point_error[point_error < self.point_threshold] = 0.0
        point_error[point_error >=
                    self.point_threshold] = self.point_threshold+0.1
...
def fit_plane_ransac(points, vectors, verticals, point_threshold=1.2, vector_threshold=5.0):
    ...
    kernel = PlaneKernel(points - points.mean(axis=0),
                         vectors, verticals, point_threshold, vector_threshold)
    p, inliers, error = ransac(kernel, point_threshold)
    num_point = points.shape[0]
    points_inliers = points[inliers[inliers < num_point], :]
    vectors_inliers = [vectors[i-num_point]
                       for i in inliers[inliers >= num_point]]
    p = fit_plane(points_inliers - points_inliers.mean(axis=0),
                  vectors_inliers, verticals)
...
def fit_plane(points, vectors, verticals):
    ...
    # Normalize the scale to improve conditioning.
    s = 1. / max(1e-8, points.std())
...
def fit_similarity_transform(p1, p2, max_iterations=1000, threshold=1):
    ...
    if len(inliers) > dim+3:
        best_T = tf.affine_matrix_from_points(
            p1[inliers, :].T, p2[inliers, :].T, shear=False)
